...
class TestArtifactLinker:
    ...
    def sample_artifact(self):
        ...
        return TrainingArtifact(
            ...
            hyperparameters={"learning_rate": 0.001, "epochs": 100},
            training_metadata={"dataset": "btc_1h_2024", "trained_at": "2024-01-15T10:00:00"},
        )
    ...
    def test_link_training_run(self, linker, mock_registry, sample_artifact):
        ...
        # Execute
        result = linker.link_training_run("grid_btc_1h", sample_artifact, tags=["training-run"])
    ...
    def test_update_with_validation(self, linker, mock_registry):
        ...
        mock_registry.update_version_metadata.return_value = mock_version
        ...
        mock_registry.update_version_metadata.assert_called_once()
        call_args = mock_registry.update_version_metadata.call_args
        assert call_args.args[0] == "v1"
        assert "validation_results" in call_args.args[1]
    ...
    def test_get_training_lineage(self, linker, mock_registry):
        ...
        mock_version.version_id = "v1"
        mock_version.status = ModelStatus.DEVELOPMENT
...
class TestTrainingIntegration:
    ...
    def test_on_training_complete_success(self, integration, mock_registry, sample_training_data):
    ...
    def test_on_training_complete_missing_metrics(self, integration, mock_registry, sample_training_data):
    ...
    def test_on_training_complete_validation_threshold_fail(self, integration, mock_registry, sample_training_data):
    ...
    def test_on_training_complete_auto_promote(self, integration, mock_registry, sample_training_data):
    ...
    def test_validate_and_promote_success(self, integration, mock_registry):
        ...
        result = integration.validate_and_promote("v1", validation_metrics, promotion_criteria)
    ...
    def test_cleanup_old_versions(self, integration, mock_registry):
        ...
        # Execute
        deleted = integration.cleanup_old_versions("test_model", keep_last_n=5)
        
        # Verify
        assert len(deleted) == 5
        assert "v0" in deleted
        assert "v4" in deleted
        assert "v5" not in deleted  # Should keep v5-v9
        assert "v9" not in deleted
    
    def test_callback_invocation(self, integration, mock_registry, sample_training_data):
