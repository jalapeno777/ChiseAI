...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    --parallel)
      ...
      ;;
    --gate)
      # Run pre-push gate (fast quality check) before CI checks
      PRE_PUSH_GATE="1"
      shift
      ;;
    --no-gate)
      # Skip pre-push gate even if default is set
      PRE_PUSH_GATE=""
      shift
      ;;
    *)
...
# Optional pre-push gate: run fast quality checks before CI
if [ "${PRE_PUSH_GATE:-}" = "1" ]; then
  echo ""
  echo "===== Running pre-push gate (fast quality checks) ====="
  if [ -f "scripts/pre_push_gate.py" ]; then
    python3 scripts/pre_push_gate.py --skip-tests
    GATE_RESULT=$?
    if [ $GATE_RESULT -ne 0 ]; then
      echo "Pre-push gate failed. Fix issues before running full CI."
      exit $GATE_RESULT
    fi
    echo "Pre-push gate passed. Continuing with full CI..."
  else
    echo "Warning: scripts/pre_push_gate.py not found. Skipping gate."
  fi
fi
