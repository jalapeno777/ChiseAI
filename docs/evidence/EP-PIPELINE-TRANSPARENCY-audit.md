...
| 9   | ST-ICT-S5                | Signal Cache TTL Dedup                | 2026-04-07      | 12f578244 (batch closeout) | —          | None on disk   | ⚠️ Partial traceability (implementation predates git history, absorbed into earlier work)                                                                                                                                                                                 |
...
| 12  | ST-ICT-ST1               | ICT Signal Quality Filter             | 2026-04-07      | —                          | —          | None on disk   | ⚠️ **CORRECTED**: Status set to `in_progress`. True implementation (ICT session-based signal cache) was never done. Note incorrectly attributed ST-ICT-S4's ConfidenceFilter (0.75 threshold) to this story. ST-ICT-S4 is the actual ConfidenceFilter in SignalGenerator. |
...
| G-002  | Stories with no SHA at all                     | ST-ICT-ST3                                       | High       | REMEDIATED — ST-ICT-ST1 status corrected to `in_progress` via R1b correction branch. ST-ICT-ST3 remains without SHA (no implementation expected). ST-ICT-ST1 true implementation (ICT session-based signal cache) was never done. |
| G-003  | Stories with "already on main" but no SHA      | ST-ICT-S2                                        | Medium     | REMEDIATED — ST-ICT-S5: Batch closeout SHA 12f578244 documented. ST-ICT-S2: remains "already on main" (lower priority). Partial traceability acceptable per task requirements.                                                    |
...
1. **G-002 (HIGH Priority)**: ✅ REMEDIATED — ST-ICT-ST1 status corrected to `in_progress` via `feature/R1B-ST-ICT-ST1-ST5-status-correction`. True implementation (ICT session-based signal cache) was never done. Note incorrectly attributed ST-ICT-S4's ConfidenceFilter work. ST-ICT-ST3: remains without SHA (no implementation expected per backlog entry).
