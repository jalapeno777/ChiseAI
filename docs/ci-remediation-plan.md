...
|--------|----------|---------|-------|
| Total Issues | 32 | 26 (-6 net) | +0 new, -6 fixed |
| P0-Critical | 2 | 0 | -2 (CI-001, CI-002 fixed) |
| P1-High | 4 | 4 | No change |
| P2-Medium | 9 | 8 | -1 (CI-003 fixed) |
| P3-Low | 17 | 14 | -3 (CI-034, CI-035 moved, CI-029 fixed) |
| Fixed Issues | 3 | 6 | CI-001, CI-002, CI-003 added |
| Est. Total Effort | 2-3 days | 1-2 days | Phases 2-3 remaining |
...
6. Phases 0 and 1 COMPLETED - ready for Phase 2 (Pipeline Hardening)
...
| ID | Issue | Root Cause | Files Affected | Est. Effort | Status |
|----|-------|------------|----------------|-------------|--------|
| ~~CI-003~~ | ~~Circular import in ml module~~ | ~~Import cycle~~ | ~~src/ml/__init__.py~~ | - | **→P2** |
...
| ID | Issue | Root Cause | Files Affected | Est. Effort | Status |
|----|-------|------------|----------------|-------------|--------|
| ~~CI-003~~ | ~~SignalOutcome not exported~~ | ~~Import works via direct path, but not from src.ml~~ | ~~src/ml/__init__.py~~ | - | **FIXED** |
...
| CI-012 | Missing validation coverage | Some validation scripts not included in CI | scripts/validate_*.py | 2 hours | Open |
| CI-013 | Local CI script incomplete | local-ci-checks.sh doesn't cover all stages | scripts/local-ci-checks.sh | 1 hour | Open |
| CI-014 | Documentation gaps | CI troubleshooting docs outdated | docs/ci/ | 2 hours | Open |
| CI-015 | Grafana dashboard missing | No CI metrics visualization | infrastructure/terraform/ | 3 hours | Open |
| **CI-032** | **Redundant pipeline files** | Multiple .woodpecker/*.yaml files with overlapping configs | .woodpecker/ | 1 hour | **NEW** |
| **CI-033** | **Non-blocking pipeline steps** | Critical validation steps configured as non-blocking | .woodpecker/ci.yaml | 2 hours | **NEW** |
...
| CI-018 | Pre-commit hook gaps | Some files bypass validation | .pre-commit-config.yaml | 1 hour | Open |
...
| CI-003 | SignalOutcome not exported | Added `from .signal_outcome import SignalOutcome` to src/ml/__init__.py |
...
### Demoted Issues (0)
*No demoted issues - all previously demoted issues have been resolved.*
...
### Phase 2: Pipeline Hardening (Day 2)
...
**Fix:** Add export to src/ml/__init__.py (Phase 1)
**Status:** Low priority
...
| Phase 2: Hardening | dev | TBD | 4 hours | **READY** |
| Phase 3: Enhancements | dev | TBD | 4 hours | Optional |

**Total Estimated Duration:** 1-2 days for remaining phases (Phases 2-3)
...
- [x] Add `from .signal_outcome import SignalOutcome` to src/ml/__init__.py
- [x] Run `pytest --collect-only -q` to verify

### Phase 2 (Pipeline Hardening)
- [ ] Audit .woodpecker/*.yaml for redundancy
- [ ] Remove `failure: ignore` from critical steps
- [ ] Fix 5 status sync mismatches
- [ ] Configure compass gate as blocking
- [ ] Run full CI validation

### Phase 3 (Enhancements - Optional)
- [ ] Add CI status check to PR auto-flow
- [ ] Improve test fixtures for external services
- [ ] Update docs/ci/ with current procedures
- [ ] Add CI metrics to Grafana dashboard
