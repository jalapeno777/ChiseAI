...
class PaperKPICalculator:
    ...
    def calculate(
        ...
        total_pnl = sum(e.get("net_pnl", 0) for e in closed_entries)
        ...
        kpis = PaperTradingKPIs(
            ...
            avg_pnl_per_trade=(
                round(total_pnl / len(closed_entries), 4) if closed_entries else 0.0
            ),
...
def print_summary(kpis: PaperTradingKPIs) -> None:
    ...
    print(f"  Total PnL: {kpis.total_pnl:.4f}")
