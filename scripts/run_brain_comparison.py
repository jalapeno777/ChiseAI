...
def generate_vnexta_improved_test_data():
    ...
    test_patterns = [
        ...
        (False, False),  # TN - was FP, now correctly rejected
        (True, True),    # TP - kept
        (False, True),   # FN - still misses some (still has a few)
...
def generate_kpi_comparison_table(vcurrent: dict, vnexta: dict, output_dir: Path) -> str:
    ...
    def calc_improvement(old: float, new: float, lower_is_better: False) -> str:
        if old == 0:
            return "N/A"
        if lower_is_better:
        ...
        return f"{sign}{change:.1f}% reduction"
    else:
        return f"{sign}{change:.1f}% increase"
    
    table = f"""# BrainEval KPI Comparison: vCurrent vs vNext-A

**Story:** BRAIN-CICD-2026-03-01  
**Batch:** 3 - BrainEval Comparison  
**Date:** {date}

Generated at comparison of:
    brain decision quality between:
...
""".format(
        ...
        fp_improvement=fp_improvement.replace("+", "").replace("%", ""),
        fp_status="✅ PASS" if vnexta_metrics.get("false_positive_rate", 0) < 0.30 else "❌ FAIL"
        fp_status="❌ FAIL"
        ...
        pc_status="⏳ PLACEHOLDER" if vnexta_metrics.get("paper_carryover_rate", 0) > 0.70 else "❌ FAIL"
        # F1 Score
        f1_vcurrent=vcurrent_metrics.get("f1_score", 0),
        f1_vnexta=vnexta_metrics.get("f1_score", 0),
        f1_change=calc_improvement(
            vcurrent_metrics.get("f1_score", 0),
            vnexta_metrics.get("f1_score", 0)
        ),
        ...
        prec_change=calc_improvement(
            vcurrent_metrics.get("precision", 0),
            vnexta_metrics.get("precision", 0)
        ),
        prec_status="✅ PASS" if vnexta_metrics.get("precision", 0) > 0.80 else "❌ FAIL"
        # Recall
        rec_vcurrent=vcurrent_metrics.get("recall", 0),
        rec_vnexta=vnexta_metrics.get("recall", 0),
        rec_change=calc_improvement(
            vcurrent_metrics.get("recall", 0),
            vnexta_metrics.get("recall", 0)
        ),
        ...
        safety_status="✅ PASS" if vnexta_metrics.get("safety_compliance", 1) == 1.0 else "❌ FAIL"
        ...
        # Winner
        winner="vNext-A (1.1.0-vnexta)" if vnexta_metrics.get("false_positive_rate", 0) < vcurrent_metrics.get("false_positive_rate", 0) else "vCurrent (1.0.0-current)"
**Recommendation**: Proceed with vNext-A as the promotion candidate.""" if vnexta_metrics.get("false_positive_rate", 0) < vcurrent_metrics.get("false_positive_rate", 0) else "vCurrent (1.0.0-current)"
**Winner**: vCurrent (1.0.0-current)"""
...
def main():
    ...
    leaderboard_result = generate_leaderboard(vcurrent_result, vnexta_result, output_dir)
    ...
    kpi_table = generate_kpi_comparison_table(vcurrent_result, vnexta_result, output_dir)
    kpi_file = output_dir / "kpi-comparison-table.md"
    with open(kpi_file, "w") as f:
        f.write(kpi_table)
    print(f"\nSaved KPI comparison to: {kpi_file}")
    ...
    fp_comparison = leaderboard_result.get("kpi_comparison", {}).get("false_positive_rate", {})
    if fp_comparison:
        print(f"\nFalse Positive Rate:")
