...
#
# Usage: ./weekly_performance_report.sh [--mode=paper|live] [--end-date=YYYY-MM-DD] [--output <file>]
#
# This script triggers the weekly report generation via the reporting module.
#
...
Generate weekly performance report for paper trading operations.

OPTIONS:
    -m, --mode <mode>          Trading mode: paper or live (default: paper)
    -e, --end-date <date>      End date for week (default: yesterday)
...
EXAMPLES:
    $(basename "$0")                           # Last 7 days paper report
    $(basename "$0") -e 2026-02-17             # Week ending on specific date
    $(basename "$0") -o /tmp/weekly.md         # Save to file
...
generate_report() {
    log_step "Generating weekly report via Python module"
...
from reporting.weekly_generator import WeeklyPerformanceReport
...
async def main():
    ...
    report = await generator.generate_report(end_date=end_date, use_mock_data=${USE_MOCK})
...
main() {
    ...
    log_info "=============================================="
    log_info "ChiseAI Weekly Performance Report"
    log_info "Mode: $MODE"
    log_info "End Date: ${END_DATE:-yesterday}"
    log_info "=============================================="
    
    if [ "$DRY_RUN" = true ]; then
        log_info "[DRY-RUN] Would generate weekly report"
        log_info "[DRY-RUN] Mode: $MODE"
        log_info "[DRY-RUN] End Date: ${END_DATE:-yesterday}"
    ...
    log_success "Weekly report generation completed"
}
