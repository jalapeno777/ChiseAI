...
def check_manifest_alignment(
    skill_path: Path, prompt_frontmatters: dict[str, dict]
) -> dict:
    ...
    for name in sorted(prompt_names & manifest_names):
        ...
        if pfm_name and pfm_name != mcap.get("name"):
            issues.append(
                f'name mismatch: frontmatter="{pfm_name}" manifest="{mcap.get("name")}"'
            )
        ...
        if pfm_mc and mcap_mc and pfm_mc != mcap_mc:
            issues.append(
                f'menu-code mismatch: frontmatter="{pfm_mc}" manifest="{mcap_mc}"'
            )
