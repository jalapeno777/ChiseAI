...
class EncryptedData:
    ...
    tag: bytes | None = None  # Authentication tag for GCM
    version: int = 1
...
class KeyManager:
    ...
    def generate_key(self, key_id: str | None = None) -> str:
        ...
        expires = (
            now + timedelta(days=self._key_rotation_days)
...
class AES256Encryptor:
    ...
    def __init__(self, key_manager: KeyManager | None = None, key: bytes | None = None):
        if key_manager:
    ...
    def encrypt(
        self, plaintext: str | bytes, key_id: str | None = None
    ) -> EncryptedData:
        ...
        if self._use_cryptography:
            ...
            warnings.warn("Using insecure fallback encryption", stacklevel=2)
            key_stream = self._generate_keystream(key, nonce, len(plaintext) + 16)
            ciphertext = bytes(
                a ^ b
                for a, b in zip(plaintext, key_stream[: len(plaintext)], strict=False)
            )
    ...
    def decrypt(self, encrypted: EncryptedData) -> bytes:
        ...
        if self._use_cryptography:
            ...
            # Fallback decryption
            key_stream = self._generate_keystream(
...
def encrypt(data: str | bytes, key: bytes | None = None) -> EncryptedData:
    """Encrypt data with AES-256-GCM."""
    encryptor = AES256Encryptor(key=key)
    return encryptor.encrypt(data)


def decrypt(encrypted: EncryptedData, key: bytes | None = None) -> bytes:
