...
class DataReplicator:
    ...
    @property
    def primary(self) -> ReplicaInfo | None:
    ...
    def get_healthy_replicas(self) -> list[ReplicaInfo]:
        return [
            ...
            and r.replication_lag_ms <= self.config.max_replication_lag_ms
        ]

    def update_replica_status(
        self, replica_id: str, is_synced: bool, lag_ms: float
    ) -> None:
        ...
        replica.last_sync_time = datetime.now(UTC)
        if lag_ms <= 1000:
            replica.health_score = 100
        elif lag_ms <= 3000:
            replica.health_score = 80
        elif lag_ms <= self.config.max_replication_lag_ms:
            replica.health_score = 50
        else:
            replica.health_score = 20

    def promote_replica(self, replica_id: str) -> bool:
        if replica_id not in self._replicas:
            return False
        replica = self._replicas[replica_id]
        if replica.health_score < 50:
            return False
    ...
    def get_status(self) -> dict[str, Any]:
        return {
            ...
            "meets_requirements": len(self.get_healthy_replicas())
...
class ServiceRedundancyManager:
    ...
    def register_service(
        ...
        replication_config: RedundancyConfig | None = None,
    ) -> None:
    ...
    def unregister_service(self, service_name: str) -> bool:
        if service_name in self._services:
            ...
            if service_name in self._data_replicators:
                del self._data_replicators[service_name]
            return True
        return False

    def get_data_replicator(self, service_name: str) -> DataReplicator | None:
    ...
    def get_service_status(self, service_name: str) -> dict[str, Any] | None:
        ...
        if service_name in self._data_replicators:
            status["data_replication"] = self._data_replicators[
                service_name
            ].get_status()
        return status
    ...
    def get_all_status(self) -> dict[str, Any]:
        return {
            "services": {
                name: self.get_service_status(name) for name in self._services
            },
