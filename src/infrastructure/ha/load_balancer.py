...
class LoadBalancer:
    ...
    def update_instance_health(self, instance_id: str, status: HealthStatus) -> None:
        if instance_id in self._instances:
            ...
            self._instances[instance_id].last_check = datetime.now(timezone.utc)

    def select_instance(self, session_id: str | None = None) -> Optional[InstanceInfo]:
        if (
    ...
    def _apply_strategy(self, instances: list[InstanceInfo]) -> Optional[InstanceInfo]:
        if not instances:
            return None
...
class ConnectionContext:
    ...
    def __init__(
        ...
        self._instance: Optional[InstanceInfo] = None
        self._start_time: float = 0.0
        self._success: bool = False

    async def __aenter__(self) -> Optional[InstanceInfo]:
