...
class UptimeRecord:
    ...
    def uptime_percentage(self) -> float:
        return (self.successful_checks / self.total_checks * 100) if self.total_checks > 0 else 100.0

    def to_dict(self) -> dict[str, Any]:
        return {"start_time": self.start_time.isoformat(), "end_time": self.end_time.isoformat(),
                "total_checks": self.total_checks, "successful_checks": self.successful_checks,
                "uptime_percentage": self.uptime_percentage, "total_downtime_seconds": self.total_downtime_seconds}
...
class Alert:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.id, "service_name": self.service_name, "severity": self.severity.value,
                "message": self.message, "timestamp": self.timestamp.isoformat(), "acknowledged": self.acknowledged,
                "acknowledged_by": self.acknowledged_by, "acknowledged_at": self.acknowledged_at.isoformat() if self.acknowledged_at else None,
...
class UptimeMonitor:
    ...
    def record_check(self, service_name: str, status: HealthStatus) -> Optional[Alert]:
        if service_name not in self._targets: return None
        now = datetime.now(timezone.utc)
        target = self._targets[service_name]
        records = self._records[service_name]
        
        current_record = records[-1] if records else None
        if not current_record or (now - current_record.start_time).total_seconds() >= 3600:
    ...
    def _check_and_generate_alert(self, service_name: str, target: UptimeTarget) -> Optional[Alert]:
        now = datetime.now(timezone.utc)
        if service_name in self._last_alert_time:
            if (now - self._last_alert_time[service_name]).total_seconds() < self.config.alert_cooldown_seconds:
                return None
        
        uptime = self.calculate_uptime(service_name, target.measurement_window_hours)
        if uptime is None: return None
        
        severity, message = None, ""
        if uptime < target.critical_threshold_percentage:
            severity, message = AlertSeverity.CRITICAL, f"CRITICAL: {service_name} uptime {uptime:.2f}% < {target.critical_threshold_percentage}%"
        elif uptime < target.alert_threshold_percentage:
            severity, message = AlertSeverity.WARNING, f"WARNING: {service_name} uptime {uptime:.2f}% < {target.alert_threshold_percentage}%"
        elif uptime < target.target_percentage:
            severity, message = AlertSeverity.INFO, f"INFO: {service_name} uptime {uptime:.2f}% < {target.target_percentage}%"
        
        if severity:
            self._last_alert_time[service_name] = now
            self._alert_counter += 1
            alert = Alert(id=f"uptime-alert-{self._alert_counter}", service_name=service_name,
                         severity=severity, message=message, details={"current_uptime": uptime})
            self._alerts[service_name].append(alert)
            if len(self._alerts[service_name]) > self.config.max_alerts_per_service:
                self._alerts[service_name] = self._alerts[service_name][-self.config.max_alerts_per_service:]
            for cb in self._alert_callbacks:
                try: cb(alert)
                except Exception: logger.exception("Alert callback error")
            return alert
        return None

    def calculate_uptime(self, service_name: str, hours: int = 24) -> Optional[float]:
        if service_name not in self._records: return None
        ...
        for record in self._records[service_name]:
            if record.end_time >= cutoff:
                if record.start_time < cutoff:
                    ratio = (record.end_time - cutoff).total_seconds() / (record.end_time - record.start_time).total_seconds()
    ...
    def _cleanup_old_records(self, service_name: str) -> None:
        if service_name not in self._records: return
        cutoff = datetime.now(timezone.utc) - timedelta(days=self.config.history_retention_days)
        self._records[service_name] = [r for r in self._records[service_name] if r.end_time >= cutoff]
    ...
    def get_service_status(self, service_name: str) -> dict[str, Any]:
        ...
        return {"service_name": service_name, "target": target.target_percentage if target else None,
                ...
                "current_status": self._last_status.get(service_name, HealthStatus.UNKNOWN).value}

    def get_all_status(self) -> dict[str, Any]:
        return {"services": {name: self.get_service_status(name) for name in self._targets},
                "total_services": len(self._targets)}

    def get_recent_alerts(self, service_name: str | None = None, hours: int = 24, unacknowledged_only: bool = False) -> list[Alert]:
        cutoff = datetime.now(timezone.utc) - timedelta(hours=hours)
        alerts = []
        services = [service_name] if service_name else list(self._alerts.keys())
        for svc in services:
            for alert in self._alerts.get(svc, []):
                if alert.timestamp >= cutoff and (not unacknowledged_only or not alert.acknowledged):
                    alerts.append(alert)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"status": self.get_all_status(), "recent_alerts": [a.to_dict() for a in self.get_recent_alerts(hours=1)],
                "config": {"check_interval_seconds": self.config.check_interval_seconds, "history_retention_days": self.config.history_retention_days}}
