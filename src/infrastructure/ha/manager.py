...
class HighAvailabilityManager:
    ...
    def register_service(
        ...
        self._uptime_monitor.register_target(
            UptimeTarget(
                service_name=service_name,
                target_percentage=self.config.uptime_target_percentage,
        ...
        self._health_registry.register(
            HealthCheckConfig(
                name=f"{service_name}_health",
                check_func=lambda: self._check_service_health(service_name),
    ...
    def _on_health_check_result(self, name: str, result: HealthCheckResult) -> None:
        if name.endswith("_health"):
            ...
            if self.config.failover_enabled:
                self._failover_manager.update_instance_health(
    ...
    def get_recent_alerts(self, hours: int = 24) -> list[dict[str, Any]]:
        return [
            a.to_dict() for a in self._uptime_monitor.get_recent_alerts(hours=hours)
        ]
...
def get_ha_manager() -> HighAvailabilityManager:
    ...
    if _global_manager is None:
        _global_manager = HighAvailabilityManager()
