...
class InstanceInfo:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.id, "host": self.host, "port": self.port, "priority": self.priority,
                "weight": self.weight, "is_primary": self.is_primary, "is_active": self.is_active,
                "last_check": self.last_check.isoformat() if self.last_check else None,
                "health_status": self.health_status.value, "metadata": self.metadata}
...
class FailoverManager:
    ...
    @property
    def active_instance(self) -> Optional[InstanceInfo]:
        return self._instances.get(self._active_instance_id) if self._active_instance_id else None
    ...
    def register_instance(self, instance: InstanceInfo) -> None:
        ...
        if instance.is_primary and (len(self._instances) == 1 or self._state == FailoverState.UNAVAILABLE):
    ...
    def update_instance_health(self, instance_id: str, status: HealthStatus) -> Optional[FailoverState]:
        if instance_id not in self._instances: return None
        ...
        # Check for failover
        if self._active_instance_id == instance_id and self._should_failover(instance_id):
    ...
    def _should_failover(self, instance_id: str) -> bool:
        if self._last_failover_time:
            ...
            if elapsed < self.config.cooldown_seconds: return False
        return self._consecutive_failures.get(instance_id, 0) >= self.config.failure_threshold

    def _should_failback(self, instance_id: str) -> bool:
        if not self._last_failover_time: return False
        elapsed = (datetime.now(timezone.utc) - self._last_failover_time).total_seconds()
        if elapsed < self.config.failback_delay_seconds: return False
        return self._consecutive_successes.get(instance_id, 0) >= self.config.recovery_threshold
    ...
    def _activate_instance(self, instance_id: str) -> None:
        ...
        if instance:
            ...
            if instance.is_primary: self._state = FailoverState.PRIMARY_ACTIVE
            logger.info(f"Activated instance: {instance_id}")

    def _get_primary_instance(self) -> Optional[InstanceInfo]:
        for inst in self._instances.values():
            if inst.is_primary: return inst
        return None

    def _get_best_backup_instance(self) -> Optional[InstanceInfo]:
        candidates = [i for i in self._instances.values() if not i.is_primary and i.health_status == HealthStatus.HEALTHY]
    ...
    def force_failover(self, target_instance_id: str | None = None) -> bool:
        if target_instance_id:
            ...
            if not target or target.health_status != HealthStatus.HEALTHY: return False
            if self._active_instance_id and self._active_instance_id in self._instances:
                self._instances[self._active_instance_id].is_active = False
    ...
    def get_status(self) -> dict[str, Any]:
        return {"state": self._state.value, "active_instance": self.active_instance.to_dict() if self.active_instance else None,
                "failover_count": self._failover_count, "last_failover": self._last_failover_time.isoformat() if self._last_failover_time else None,
