...
def generate_client_order_id(symbol: str, id_length: int = 8) -> str:
    ...
    random_component = ''.join(
        secrets.choice(alphabet) for _ in range(id_length)
    )
...
def build_idempotency_key(symbol: str, client_order_id: str, prefix: str = IDEMPOTENCY_KEY_PREFIX) -> str:
...
class IdempotencyStore:
    ...
    async def check_duplicate(
        ...
        key = build_idempotency_key(
            symbol, client_order_id, self.config.key_prefix
        )
