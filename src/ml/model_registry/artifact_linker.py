...
class ArtifactLinker:
    ...
    def update_with_validation(
        ...
        # Update the version
        updated = self.registry.update_version_metadata(version_id, metadata)
        logger.info(f"Updated version {version_id} with validation results")
    ...
    def get_training_lineage(self, version_id: str) -> Dict[str, Any]:
        ...
        lineage = {
            ...
            "training_artifact": version.metadata.get("linked_artifact", False) if version.metadata else False,
        }
